...
def search(start: Vertex.Vertex, goal: Vertex.Vertex, graph: Graph.Graph, h):
    return path_search_algorithms.best_first_search.search(graph.vert_dict[start.get_id()], graph.vert_dict[goal.get_id()], graph, lambda x, y: x + y, lambda x, y: graph.vert_dict[x.get_id()].get_weight(y), h);
